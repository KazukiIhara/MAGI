...
void MeshDrawer::Draw(uint32_t instanceCount) {
	...
	/* b1 : Material (root param 2) */
	cmd->SetGraphicsRootConstantBufferView(2, materialBuffer_->GetGPUVirtualAddress());

	/* t1–t5 : SRV バインド */
	cmd->SetGraphicsRootDescriptorTable(5, MAGISYSTEM::GetSrvUavDescriptorHandleGPU(vertexSrvIdx_));     // t1
	cmd->SetGraphicsRootDescriptorTable(6, MAGISYSTEM::GetSrvUavDescriptorHandleGPU(indexSrvIdx_));      // t2
	cmd->SetGraphicsRootDescriptorTable(7, MAGISYSTEM::GetSrvUavDescriptorHandleGPU(meshletSrvIdx_));    // t3
	cmd->SetGraphicsRootDescriptorTable(8, MAGISYSTEM::GetSrvUavDescriptorHandleGPU(uniqueVertSrvIdx_)); // t4
	cmd->SetGraphicsRootDescriptorTable(9, MAGISYSTEM::GetSrvUavDescriptorHandleGPU(primSrvIdx_));       // t5
