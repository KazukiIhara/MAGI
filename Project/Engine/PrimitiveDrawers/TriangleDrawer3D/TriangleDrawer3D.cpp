...
TriangleDrawer3D::TriangleDrawer3D(DXGI* dxgi, DirectXCommand* directXCommand, SRVUAVManager* srvUavManager, GraphicsPipelineManager* graphicsPipelineManager, Camera3DManager* camera3DManager) {
	...
	// 最大数分確保
	triangles_.resize(PrimitiveCommonConst::kNumMaxInstance);
	materials_.resize(PrimitiveCommonConst::kNumMaxInstance);
...
void TriangleDrawer3D::Update() {
	// 上限を超えていたらassert
	assert(currentIndex_ <= PrimitiveCommonConst::kNumMaxInstance);
...
void TriangleDrawer3D::AddTriangle(
	...
	if (currentIndex_ >= PrimitiveCommonConst::kNumMaxInstance) {
...
void TriangleDrawer3D::CreateInstancingResource() {
	// instancing用のリソースを作る
	instancingResource_ = dxgi_->CreateBufferResource(sizeof(TriangleData3DForGPU) * PrimitiveCommonConst::kNumMaxInstance);
	// srvのインデックスを割り当て
	instancingSrvIndex_ = srvUavManager_->Allocate();
	// Srvを作成
	srvUavManager_->CreateSrvStructuredBuffer(instancingSrvIndex_, instancingResource_.Get(), PrimitiveCommonConst::kNumMaxInstance, sizeof(TriangleData3DForGPU));
}
