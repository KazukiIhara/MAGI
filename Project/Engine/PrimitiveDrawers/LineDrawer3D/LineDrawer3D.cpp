...
LineDrawer3D::LineDrawer3D(DXGI* dxgi, DirectXCommand* directXCommand, SRVUAVManager* srvUavManager, GraphicsPipelineManager* graphicsPipelineManager, Camera3DManager* camera3DManager) {
	...
	// Srvを作成
	srvUavManager_->CreateSrvStructuredBuffer(srvIndex_, instancingResource_.Get(), PrimitiveCommonConst::kNumMaxInstance, sizeof(LineData3D));

	lines_.reserve(PrimitiveCommonConst::kNumMaxInstance);
...
void LineDrawer3D::Update() {
	// 最大数を超えていたら止める
	if (lines_.size() > PrimitiveCommonConst::kNumMaxInstance) {
...
void LineDrawer3D::CreateInstancingResource() {
	// instancing用のリソースを作る
	instancingResource_ = dxgi_->CreateBufferResource(sizeof(LineData3D) *PrimitiveCommonConst::kNumMaxInstance);
}
...
void LineDrawer3D::MapInstancingData() {
	...
	for (uint32_t index = 0; index < PrimitiveCommonConst::kNumMaxInstance; ++index) {
