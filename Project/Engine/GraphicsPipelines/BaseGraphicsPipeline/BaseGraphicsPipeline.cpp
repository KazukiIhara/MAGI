...
ID3D12RootSignature* BaseGraphicsPipeline::GetRootSignatureShadow() {
	return rootSignatureShadow_.Get();
}

ID3D12PipelineState* BaseGraphicsPipeline::GetPipelineStateShadow() {
	return pipelineStateShadow_.Get();
}
...
void BaseGraphicsPipeline::CreateGraphicsPipelineObjectShadow() {
	assert(rootSignatureShadow_);
	...
	D3D12_SHADER_BYTECODE meshShader = { meshShaderBlobShadow_->GetBufferPointer(), meshShaderBlobShadow_->GetBufferSize() };
	D3D12_SHADER_BYTECODE pixelShader = { pixelShaderBlobShadow_->GetBufferPointer(), pixelShaderBlobShadow_->GetBufferSize() };
	...
	Primitive3DPipelineStateStream stream = {
		CD3DX12_PIPELINE_STATE_STREAM_ROOT_SIGNATURE(rootSignatureShadow_.Get()),
		CD3DX12_PIPELINE_STATE_STREAM_AS(amplificationShader),
		CD3DX12_PIPELINE_STATE_STREAM_MS(meshShader),
		CD3DX12_PIPELINE_STATE_STREAM_PS(pixelShader),
		CD3DX12_PIPELINE_STATE_STREAM_RASTERIZER(rast),
	...
	HRESULT hr = dxgi_->GetDevice10()->CreatePipelineState(&streamDesc, IID_PPV_ARGS(&pipelineStateShadow_));
	if (FAILED(hr)) {
