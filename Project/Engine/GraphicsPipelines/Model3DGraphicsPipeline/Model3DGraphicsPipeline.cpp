...
void Model3DGraphicsPipeline::Initialize() {
	BaseGraphicsPipeline::Initialize();
	CreateRootSignatureShadow();
	CreateGraphicsPipelineObjectShadow();
}
...
void Model3DGraphicsPipeline::CreateRootSignatureShadow() {
	HRESULT hr;
	...
	hr = dxgi_->GetDevice()->CreateRootSignature(0, sigBlob->GetBufferPointer(), sigBlob->GetBufferSize(), IID_PPV_ARGS(&rootSignatureShadow_));
	assert(SUCCEEDED(hr));
}
