#include "Cylinder3DGraphicsPipeline.h"
...
void Cylinder3DGraphicsPipeline::CreateRootSignature() {
	HRESULT hr;
...
void Cylinder3DGraphicsPipeline::CompileShaders() {
	...
	meshShaderBlobWithAlpha_ = shaderCompiler_->CompileShader(L"EngineAssets/Shaders/Graphics/Cylinder3D/Cylinder3DWithAlpha.MS.hlsl", L"ms_6_5");
...
void Cylinder3DGraphicsPipeline::CreateGraphicsPipelineObject() {
	...
	for (uint32_t i = 0; i < kBlendModeNum; ++i) {
		...
		if (FAILED(hr)) {
			Logger::Log("Cylinder3DGraphicsPipeline: Failed to create PSO for blendMode " + std::to_string(i));
			assert(false);
		}
	}
}

D3D12_BLEND_DESC Cylinder3DGraphicsPipeline::BlendStateSetting(uint32_t blendModeNum) {
...
D3D12_DEPTH_STENCIL_DESC Cylinder3DGraphicsPipeline::DepthStecilDescSetting() {
...
D3D12_INPUT_LAYOUT_DESC Cylinder3DGraphicsPipeline::InputLayoutSetting() {
	return { nullptr, 0 };
}

D3D12_RASTERIZER_DESC Cylinder3DGraphicsPipeline::RasterizerStateSetting() {
